...
public class ImageUtil {
	...
	public static HashMap<Integer, Model> getMediaImage(Context context) {
		...
		for (int i = 0; i < imagecursor.getCount(); i++) {
			...
			Model galleryModel = new Model("file:/" + imagecursor.getString(dataColumnIndex).toString(), false);
	 ...
	 * @return
	 */
	public static HashMap<Integer, Model> getHttpImage(Context context) {
		...
		for (int i = 0; i < imageUrls.length; i++) {
			datas.put(i, new Model(imageUrls[i], false));
		}
		return datas;
	}
}
