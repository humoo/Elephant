...
import com.example.elephant.image2.BitmapCache.ImageCallback;
...
//				if ((Bimp.bmp.size() + selectTotal) < 9) {//-------判断已选张数有错
				if (AddNewLiaotieActivity.picCount < 9) {//-------判断已选张数有错
					...
					if (item.isSelected) {
						...
						map.put(path, path);
						Log.d("logd", "Adapter-Map:"+path);
//						Log.d("logd", map.size()+"");
						AddNewLiaotieActivity.picCount=AddNewLiaotieActivity.picCount+1;
						AddNewLiaotieActivity.myMap.put(path, path);
...
//						holder.selected.setImageResource(-1);
						...
						AddNewLiaotieActivity.myMap.remove(path);
						AddNewLiaotieActivity.picCount=AddNewLiaotieActivity.picCount-1;

					}	
//				} else if ((Bimp.bmp.size() + selectTotal) >= 9) {
				} else if (AddNewLiaotieActivity.picCount >= 9) {
